...
- TUI `/career` and the existing `/career-setup`, `/career-library`, `/career-application`, `/career-vacancy`, `/career-match`, `/career-analyze`, `/career-workbench`, and `/career-workspace` commands open one Career overlay at different views; RPC dialogs remain the fallback and overlay navigation does not call Core or a provider.
