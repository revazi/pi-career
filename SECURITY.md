...
Global config stores only canonical resume-root paths, bounded labels, an optional variation-directory suggestion, and an optional application-root UUID/path binding. First ordinary setup may create only the missing agent-directory-relative `career/` directory as canonical effective-user-owned `0700` beneath an existing canonical Pi agent directory; pre-existing unsafe objects are never adopted or chmodded, and workspace configuration requires that private directory already exist. Owner-only `0600` config locks plus byte/inode compare-and-swap protect writes. Ordinary absent/v1 mutations remain canonical v1 while no workspace is attached; only confirmed **Configure application root** migrates to strict v2, after which every writer preserves v2 and freshly rejects physical overlap. Scans are bounded, UTF-8 strict, and symlink-free. Searchable PDFs are limited to 10 MiB and 20 pages and parsed in a package-owned worker with fixed time, memory, and result ceilings. Network fetch, eval, WASM, streaming, auto-fetch, and system-font lookup are disabled; OCR is not performed. Source text and full Core results are never written to global config or logs. User-only no-argument `/career-workspace` Gate 1 may separately persist a non-content root marker/manifest, append-only full status metadata, exact explicitly approved workflow-original vacancy bytes, and selected-original digests under one existing owner-only `0700` root disjoint from every resume/variation root. Every mutation requires unchanged complete preview plus separate confirmation and uses bounded private no-clobber publication; workspace actions invoke no Core/runtime/provider/model/network/child process and append no session entry. Assisted workspace artifacts, deletion, repair/adoption/archive/sync, and broader files are not implemented.
