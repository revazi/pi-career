...
`/career-vacancy`, `/career-match`, and `/career-analyze` call that adapter helper directly; setup/library scanning and application state remain local. `/career-workbench` does not call a provider, model, or Core process; it only prepares a bounded ordinary Pi editor message that visibly contains the selected private source. Nothing is sent automatically. If the user submits it, Pi may persist it as a user message and send it to the selected provider, whose retention policy is outside this package. The workbench omits absolute source paths, keeps originals immutable, and states that extracted PDF text cannot expose or preserve visual layout.

The workflow's global config stores only canonical root paths and bounded labels in an agent-directory-relative `0700` directory with atomic `0600` writes. Scans are bounded, UTF-8 strict for Markdown/text, and symlink-free. Searchable PDFs are limited to 10 MiB and 20 pages and parsed from memory by a package-owned worker with fixed time, memory, and result ceilings. Network fetch, eval, WASM, streaming, auto-fetch, and system-font lookup are disabled; malformed, encrypted, image-only, oversized, or timed-out PDFs fail with a generic local notice. OCR is not performed. Source text and full Core results are never written to global config or logs.
...
Pi may store tool arguments/results and workflow custom entries in session JSONL. Keep one company/role application per Pi session and use `/new` before another application so prior private workbench messages do not remain in the next provider context. Before the workflow appends its first private vacancy or compact result card in a persisted session, or prepares private workbench text for that session's editor, it requires the explicit local-persistence decision. Custom workflow entries do not enter LLM context, and full Core JSON is not stored. Direct private native-tool use still requires an explicit decision. Prefer a new transient run when persistence is not wanted:
