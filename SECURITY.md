...
The pinned development tool tree currently has a nonzero full npm audit. Its exact report, installed vulnerable package versions/paths, advisory details, lockfile, counts, npm version, and fixed expiry are temporarily accepted by the checked-in validator through (but not at) `2026-08-19T00:00:00.000Z`. Strict bounded parsing rejects duplicate decoded keys before comparison. This is development/host-Pi risk acceptance, not a clean full audit or a blanket waiver. A different host Pi tree is not covered. Any drift, duplicate-key ambiguity, untrusted tool coordinate, command failure, malformed report, or expiry makes the direct Node `check:publish` orchestrator fail closed.
