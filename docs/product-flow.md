...
- `career_run variant-review` terminates its turn; the TUI selector defaults every retained change to excluded, pages exact before/after/evidence locally, and prepares rather than submits a later materialization request
- review selection is process-memory-only: pi-career appends no selection entry, performs no Core call, and puts only the ephemeral review handle and selected canonical IDs in the editor
