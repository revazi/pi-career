...
The cross-repository migration plan and measured findings are recorded in [`career-run-roadmap.md`](career-run-roadmap.md). Career Core Phase 8, the first managed `career_run` implementation, and the threat-reviewed package-owned Markdown/text save workflow in [`variant-save-workflow.md`](variant-save-workflow.md) are complete. Synthetic PDF compatibility covers reviewed Chromium-generated tagged/untagged and ReportLab-generated two-page documents, embedded-font and multi-column layouts, and fail-closed text-unavailable cases. The current next phase freezes a Git-tree/hash-bound pre-optimization context benchmark—including Pi Skill discovery and matching-task full-Skill context—before any model-visible token optimization. A distinct Git-history-anchored optimizing state lets that optimization merge before a separate after snapshot is bound to the optimization merge commit. Full application-workspace persistence and further PDF extraction expansion remain later phases.
