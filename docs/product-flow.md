...
Automatic variant saving remains deferred. Setup now records or derives only a preferred destination suggestion: an explicit override wins, otherwise `variants/` under the selected original's configured root is recommended. It creates no directory or file. The reviewed design target requires a future user-only `/career-save <variant-handle>` command, exact unchanged preview, separate confirmation, no-clobber private writes, strict assisted sidecars, and fail-closed rescanning; it does not authorize implementation. Any implementation must not add provider fallback, automatic repair/retry, overwrite originals, auto-apply changes, or feed assisted output back into authoritative analysis/matching.
