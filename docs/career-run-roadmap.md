...
Application attachment and model-context activation are separate. #64 fixes the exact strict custom-entry data in [`application-workspaces.md`](application-workspaces.md): one path-free root/application identity pointer, an exact detachment record, and a separate activation bound to that attachment. Implementation removes static Skill discovery, reconstructs valid activation in `session_start`, conditionally contributes the bundled Skill path from `resources_discover`, activates only `career_run`, and uses one terminal reload after activation, detach, or a branch visibility change. It must follow Pi’s fresh-session lifecycle and never retain an old context after session replacement.
