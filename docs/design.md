...
| Automatic, ambiguous, or stale review selection | Variant review terminates the turn; exact warnings/discards and per-change evidence are paged locally; all IDs default excluded; only user-included canonical IDs in Core order enter a later editor message; stale/PDF handles fail closed. |
