...
`/career-tools managed|raw|status` controls whether the raw tools occupy active model context; it never changes the verified process/runtime boundary.
...
[`../runtime/manifest.json`](../runtime/manifest.json) pins unsigned artifacts from Career Core commit `f6d17835de4817e28ce84e8e5734ff592687dfcc` and native artifact run [31274605956](https://github.com/revazi/career-core/actions/runs/31274605956). It records exact binary/provenance paths, target triples, sizes, modes, SHA-256 values, source archive identities, shared notices, operation-catalog/schema-bundle/output-bound compatibility evidence, and representative-operation digests. Imported per-target provenance remains package-bounded.
...
| Oversized request, stream, diagnostic, memory, or context | Core-catalog input ceilings before spawn, independent stdout/stderr ceilings, fixed timeout, raw 50,000-byte/2,000-line limits, managed 32-MiB complete capture, a 16-entry/64-MiB ephemeral registry, and 50,000-byte model projections/details; every path fails closed without authoritative truncation. |
...
| Adapter changes Core authority | Raw successful JSON remains complete and unchanged apart from one framing newline. Managed projections are derived only after complete capture, retain all warnings plus operation-relevant uncertainty/authority/discard fields, expose exact sections/items through handles, and never become authoritative Core contracts. |
...
The initial process adapter, tool mappings, synthetic fake executable, process tests, and skill guidance were adapted from Career Core commit `100774ed8bb3b39c019d64ce105af1e3f209144f`. Bundled runtime artifacts and their managed-adapter compatibility metadata were imported from exact Core commit `f6d17835de4817e28ce84e8e5734ff592687dfcc` via reviewed native artifact run `31274605956`. This repository preserves the applicable `MIT OR Apache-2.0` terms and package-bounded notices in `NOTICE` and `runtime/`.
