...
Acquisition invokes the npm CLI derived from the real `process.execPath` installation using the reviewed Unix or Windows Node installation layout, rejects ambiguous or conflicting `npm_execpath`, pins the canonical registry/package/version, includes optional native packages, and disables lifecycle scripts. It never invokes an npm/npx executable selected from caller `PATH`. Unix acquisition pins `/bin/sh`; Windows acquisition validates `SystemRoot`, derives `System32/cmd.exe`, rejects conflicting `ComSpec`, sanitizes shell/PATH overrides, and uses that shell only inside npm's acquisition-only execution. npm's temporary `.bin` entry is used only to locate the exact installed `@revazi/career/bin/career.js`. The locator returns one versioned, canonical base64 marker inside bounded stdout so platform-specific npm banner text can be ignored without becoming an executable path or exposed diagnostic; pi-career then invokes the decoded, manifest-validated JavaScript launcher directly with real Node and `shell: false`.
...
`test:career-package` separately exercises exact-package acquisition, managed Core 0.1.1 compatibility, and representative synthetic resume/job operations. Hosted CI runs that acceptance natively for all eight launcher targets: both macOS architectures, GNU Linux x64/ARM64, musl Linux x64/ARM64 in the exact pinned Alpine/Node container on native hosts, and Windows MSVC x64/ARM64. The original macOS arm64 and Linux x64 GNU contexts continue to run the complete release ladder; additional target jobs run build/type/runtime-boundary, exact-package, offline-load, and diff checks appropriate to their native environment.
