...
- `tests/workflow/no-automatic-provider-submission.test.mjs` witnesses P3-32 through a real installed RPC child and a reachable loopback provider trap. Browsing lists and opens a synthetic catalog application without attachment or session append. Confirmed no-Core mutations cover identity migration, persistent package creation, attached status revision, and selected-original binding, including preview cancellation and confirm denial. The trap request count stays zero after a positive probe, host agent/turn/message events stay absent, messages stay empty, raw tools stay inactive, and private bytes/modes outside the approved files stay exact. Core-routed vacancy/analyze/match, config-root and library-root mutations, Ask Pi editor handoff, and process shutdown are not this witness.
