...
- `tests/workflow/no-automatic-provider-submission.test.mjs` witnesses P3-32 through a real installed RPC child and a reachable loopback provider trap. Browsing lists and opens a synthetic catalog application without attachment or session append. Confirmed no-Core mutations cover identity migration, persistent package creation, attached status revision, selected-original binding, and overlay Detach, including preview cancellation, confirm denial, and detach cancellation. Confirmed Detach appends only the exact session detachment, retains the used-application claim, and leaves workspace files unchanged; inactive assistance means this path does not reload. The trap request count stays zero after a positive probe, host agent/turn/message events stay absent, messages stay empty, raw tools stay inactive, and private bytes/modes outside the approved files stay exact. Core-routed vacancy/analyze/match, config-root and library-root mutations (including workspace-root detach), Ask Pi editor handoff, activated detach reload, and process shutdown are not this witness.
