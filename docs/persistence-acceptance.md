...
- `tests/workflow/application-readiness-integration.test.mjs` exercises the attached read-only state-to-readiness boundary with independently canonicalized synthetic package and library bytes. It establishes a mixed-chain Ready package, then performs separate exact-parent job-description and selected-original transitions, proving a stale bound letter, `Incomplete 2/3`, applied/closed lifecycle non-bypass, preserved history, private modes, and mutation-free derivation. It adds no cover-letter writer or provider/Core/session path; broader filesystem races and overlay presentation remain #65 and later overlay slices. P3-16 remains deferred: registered creation has a deliberate session creation-entry/session-name effect, but no integrated witness yet establishes configured-root package bytes, exact preview and consent, restart-like overlay readiness, and the bounded absence of every other effect.
