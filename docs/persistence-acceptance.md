...
- The earlier `P3-08` reconciliation test in `tests/workflow/application-workspace.test.mjs` remains supporting evidence for repeated v1 reconciliation. The #63 cases separately exercise non-authoritative legacy slug projection, exact identity-only migration, cancellation, state/session races, blocked pre-migration writers, and unchanged historical bytes. Mutation-triggered v1→v2 state transition is not treated as explicit identity migration, so P3-11 and P3-47 remain deferred.
- Other ledger rows remain specifications, not newly implemented tests. The 25 deferred rows require issue-owned behavioral integration; the map intentionally refuses to count fixture-only or narrower read evidence as completed mutation/UI coverage. #60 remains open pending review of these deferrals and further end-to-end coverage.
