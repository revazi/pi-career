...
- `tests/workflow/transient-process-shutdown-acceptance.test.mjs` witnesses P3-30 with a real RPC child and P3-58 with a dependency-free native-TUI PTY child. The PTY uses the public Applications and Workbench UI for attachment/activation, confirms Ask Pi, separates activation’s reload from a later explicit editor `/reload`, and observes two native reload lifecycle/resource cycles. A delayed public observer records the reconstructed Skill/tools/entry surface after resources discovery and records no agent, turn, message, tool-execution, or provider-request events; same-pid retention, shutdown loss, fresh-process absence, and exact private trees are asserted.
