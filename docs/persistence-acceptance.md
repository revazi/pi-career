...
| P3-16 | Configured root and approved company/role | Create application through registered Applications workflow | Preparing, Incomplete 0/3; no automatic attachment or effects beyond explicitly authorized creation entry | #55 |
...
The ledger preserves all **58** approved Spec 3 IDs (P3-01–P3-58); the first 48 are not the entire current approved map. The #60 disposition test assigns each row exactly once to a behavioral witness or a scenario-specific deferred rationale; the Owner column supplies its owning issue. Currently 55 have scoped witnesses and 3 remain deferred. P3-05 is witnessed by registered configure-root and `/career` Applications discovery/attach on a marked private root with one valid application, one legacy application, one colliding UUID pair, and one unknown direct child. The unknown child remains aggregate drifted and is not listed, attached, adopted, rewritten, removed, or repaired. Configure-root and detach fail closed with the existing payload-free workspace errors and do not preview or mutate; another root path remains unavailable. Only the valid pointer is attachable. Legacy and collision restrictions remain. Repeated actions preserve config, root, application, library, and session names, bytes, SHA-256 values, and private modes, with no Core resolution, send, activation, new session, or prompt/result persistence. P3-06/P3-07 are witnessed by one registered `/career` Applications list and local open/detail flow after restart-like reconstruction. Independently canonical synthetic packages keep exact company/role bytes in identity files while directory slugs differ; two distinct UUIDs retain Unicode-equivalent but byte-different labels without normalization or conflation. Repeated RPC and TUI reads preserve config, root, application, library, and session names, bytes, and private modes. Invalid and colliding children stay under existing catalog classifications and are not adopted. This local witness does not establish provider-wire or process-shutdown evidence. P3-16 is witnessed by the registered `/career` Applications create action on a persisted session: consent is resolved before the existing initialize preview, denial and preview cancellation publish nothing, and a registration-scoped post-preview root drift fails closed with no package, lock, temporary file, application entry, or session name. Approved creation writes one canonical private package and then the creation entry; a fresh empty session renders that catalog row as Preparing, Incomplete 0/3. This is local synthetic acceptance, not provider-wire or process-shutdown evidence. P3-36 is witnessed by installed `pi --print` and `pi --mode json --print` invocations of registered `/career` in a child process whose Node permission policy allows package loading and host-required agent settings, while independently denying existing synthetic private config, session, application-root, library, and document paths. Both modes emit only the stable payload-free `interactive_mode_required` error before those reads, with no Core invocation, provider turn, or private-byte output. Registration-scoped session, UI, append, send, and Core traps remain companion evidence. This is not a resumed-session host load or provider-wire witness. P3-37 is witnessed by registered and installed payload-free error boundaries covering resolver probes, Core invocation, source scan/extraction/validation, catalog/config/workspace reads, session loader/replay, UI select/editor/notification failures, persistence/commit failures, and public managed/raw tool errors. Each public result is an approved stable code or message; cancellation stays distinguishable from generic failure. Stdout, stderr, RPC UI, session entries, result cards, and synthetic config/root/library/package/lock/temp sinks contain no injected sentinel, raw stack, path, environment value, private UUID/hash/body/schema bytes, or errno text. No logger or telemetry was added. P3-29/P3-54 have one synthetic registered Applications-flow witness covering both materially distinct claim locations: A attached on the active branch, and A attached on a replayable sibling branch while the active branch follows a different child from their common ancestor and contains no application claim. A public request to attach independently valid B emits the fixed payload-free conflict notice and explicitly offers `Open application in new Pi session`; declining leaves all old entries and private config/application bytes unchanged. Explicit acceptance alone calls `ctx.newSession`, seeds only B's attachment through replacement setup, and leaves A's original session/history unchanged. Core, model-send, tool-activation, document-injection, and workspace-tree spies remain unchanged at available local boundaries; this does not claim provider-wire or network interception. P3-46 has a synthetic combined Applications view witness: a validated unattached current-session row is marked Not persisted beside persistent records, with read-only TUI/RPC navigation and rescan, unchanged session authority and private-tree bytes, and no Core or automatic send. UUID duplicates and invalid or attached session identities do not create a second row. This local witness does not establish provider-wire behavior. P3-22 has independent synthetic missing and changed selected-original cases at the public attached-source boundary while another eligible original remains. The registered attached Match command and direct public loader remain fail-closed: exact scans distinguish absent A from same-identity/different-digest A, B remains independently eligible, fixed payload-free `workspace_drift` rejects validation, and Core/send spies, full config/library/application snapshots, and session snapshots show no fallback or side effect. P3-23 independently constructs canonical private artifact and sidecar bytes whose exact hashes are referenced by state, while the sidecar binds to eligible original B and state still selects eligible original A. Catalog classification is drifted and public attached-source validation returns fixed payload-free `attachment_unavailable`; registered attached Match reaches neither Core nor send, and complete snapshots prove no fallback, append, or file mutation. These are local public-boundary witnesses, not provider-wire interception. P3-48 has a synthetic mixed v1/v2 application-state chain linking a selected original to an assisted artifact, with a library-visible sidecar-classified copy. Scanner and RPC original selectors exclude that copy; an attached application cannot rebind an original once its artifact is linked. Attached Analyze sends only original bytes to Core, while attached Match intentionally sends the validated effective assisted Resume. Changed artifact bytes and a sidecar bound to a different original block Match without adoption or fallback. The earlier unattached selector test remains a narrower regression; neither test establishes provider-wire behavior. P3-35 has a synthetic RPC/TUI local-preview witness: metadata detail remains body-free until the explicit preview action; eligible Markdown, text, and extracted searchable PDF originals are freshly validated, bounded, and displayed exactly on the transient local surface. Companion attached-workspace UI tests cover current vacancy, selected original, and assisted-labeled effective Resume, with byte/entry/Core checks, stale-source rejection, and exclusion of library assisted variants as originals. This is local UI evidence, not provider-wire acceptance. P3-34 remains deferred and is not promoted by this witness. P3-36's installed print/JSON witness denies each private config, session, application-root, library, and document path and still reaches only the stable command error; retained in-process traps cover session getters, UI, append, send, and Core. It does not establish resumed-session host loading or provider-wire behavior. Earlier P3-37 session, UI, and Core-invoke regressions remain in `early-failure-errors` and are not a substitute for the cohesive resolver, source, catalog, persistence, and sink witness. No logger was added. P3-32 is witnessed by one live installed `pi --mode rpc --offline --no-session` process whose configured model `baseUrl` is a reachable loopback provider trap. A harness probe proves the trap counts a request before the child starts; the child then records zero requests. Registered `/career` lists and opens a synthetic catalog application with no attachment, session append, Core invocation, provider request, or agent/message turn. The same process confirms the no-Core application mutations currently exposed as distinct workspace transactions: legacy identity migration (`finish_application_migration`), persistent package creation (`initialize_application`) plus its creation entry, attached status revision (`record_state`), selected-original binding (`select_original`), and confirmed overlay Detach. Preview cancellation and confirm denial publish nothing. Overlay Detach cancellation appends nothing; confirmation appends only the exact `application_detachment` for the current attachment, retains the used-application claim, leaves workspace files unchanged, and does not reload because assistance is inactive. Approved effects are only the identity file, the new canonical package, the creation and attachment entries, the exact v2 revisions, and that one detachment entry. Vacancy, analyze, and match are excluded because they require Core. Config-root and library-root mutations, including workspace-root detach, and Ask Pi editor handoff are outside this slice; editor preparation remains owned by P3-51 and activated detach reload remains owned by P3-53. Neither is treated as submission. Transient consent is the observed notice, not a persisted-session consent dialog. This is not process-shutdown evidence. P3-33 is witnessed by registered `/career` create, migrate, and list/browse plus public readiness derivation while distinct synthetic Core-result, Core-error, provider-request, provider-response, prompt, and editor sentinels remain in process memory. List renders the in-memory result card only on the local Analyze surface. Cancellation, rejected preview, post-preview drift, and consented success are recursively snapshotted. Approved writes are canonical identity, manifest, and state files plus consent and application session entries and the session name. Config, application root, library, session file, result-card append, lock/temp/residue, synthetic package cache, and the rest of the writable temp tree contain none of those sentinel bytes. A fresh empty-memory read reconstructs readiness from disk. The loopback provider trap receives no workflow request. This is not secure erasure or provider-wire acceptance beyond that trap. P3-26, P3-27, and P3-34 exercise synthetic unattached RPC browse/open and rendered lists with session, invocation, send-path, and private-tree byte assertions; they do not establish model transport behavior or private-read interception. P3-50 and P3-57 are witnessed by isolated installed RPC children and a bounded synthetic loopback provider. P3-50 attaches a valid catalog application without activation and then sends one ordinary non-Career prompt. P3-57 requests raw tools through the registered `/career-tools raw` command, receives the stable inactive rejection with no activation or resource reload, and then sends a separate ordinary prompt. Each captured request contains that explicit prompt once and the unchanged host tool schemas, and contains no Career Skill discovery metadata, Skill content, `career_run` schema, or raw `career_core_discover`, `career_core_resume`, or `career_core_job` schema. Synthetic document, identity, path, session-entry, and editor-handoff bytes are absent. The loopback returns one bounded synthetic acknowledgement, no Career Core executable runs, and no provider request occurs before the explicit prompt. This is local synthetic provider-request evidence, not an external-provider witness. P3-52 uses two separately registered installed-extension instances over independently serialized canonical attachment/activation entries. Each fresh `session_start` revalidates the exact private application pointer, restores only active `career_run` plus the Career Skill path, leaves raw tools inactive, and preserves the complete private config/application/library/session tree; scoped Core-executable, send, append, reload, and new-session observations stay unused. P3-56 exercises malformed attachment and activation data, activation without attachment, duplicate attachment and activation transitions, detachment or activation bound to another attachment, duplicate lifecycle record UUIDs, conflicting application UUID claims, a drifted exact pointer, a missing configured pointer, and a valid pointer whose selected source binding later drifted through the same installed restore, Skill action, and direct managed-action boundaries. Invalid restore records remain inactive; the action-only source drift deactivates its initially pointer-valid surface. Every case returns the fixed inactive/unavailable state before Core resolution while complete trees and entries remain unchanged and the append API remains unused. These are local lifecycle witnesses, not provider-wire or network interception. P3-53 remains deferred because the installed session_tree handler receives ExtensionContext, which has no reload(); reload() exists only on ExtensionCommandContext, and host resources_discover runs on startup or reload rather than after tree navigation. A harness-supplied reload on that event would invent a command-context seam. Explicit detach still does not establish navigate-before-activation Skill reload or absence of provider/Core calls and handles. P3-04 has a catalog test for corrupt manifest, identity, and chain with byte preservation. P3-08 and P3-50 were previously linked to narrower tests that did not establish their full Then clauses. P3-50's provider-request witness above supersedes that narrower surface projection. P3-20/P3-21 now use actual appended current job-description and selected-original transitions from a validated Ready package. P3-30 is witnessed by one live `pi --mode rpc --offline --no-session` process that attaches through the public `/career` select/confirm surface, keeps that attachment only in memory with no session file, and is stopped with SIGTERM. A later process has a new pid, no attachment entry, and no Career Skill or managed tool surface, while approved config, root, application, and library names, bytes, SHA-256 values, and private modes stay exact. The same live pid also shows activation's in-process `ctx.reload()` retaining entries; that is not an independent TUI `/reload`, which RPC `get_commands` does not expose and which must not be prompted. P3-58 therefore remains deferred. This witness does not claim provider-wire evidence. Exact-title pointers are bookkeeping, not proof of semantic fitness; a witness represents only its exercised public boundary, not blanket end-to-end assurance. Foundation issue #65 must verify its own applicable subset and report later scenarios as pending rather than pretending #60 or #65 completes all end-to-end coverage.

P3-40 is witnessed by the registered attached vacancy mutation and registered `Status and reconcile` action across restart-like reconstruction. A registration-scoped checkpoint throws only after the exact approved replacement vacancy has been no-clobber published, synchronized, privately verified, and had its temporary file removed, but before its state revision is published. The fixed failure retains only that orphan; independently canonical manifest, identity, historical state, and historical vacancy bytes remain exact. Reconciliation recognizes the extra managed artifact against the fully validated committed chain but performs no adoption, write, cleanup, rename, append, Core call, send, session creation, tool activation, or fallback. Catalog and attached-source reads continue to fail closed, so the orphan is never current, attached, selected, or effective. Complete config/root/application/library/session snapshots include names, bytes, SHA-256 values, and private modes. This local synthetic witness is distinct from exact-state settlement and does not claim provider-wire interception. P3-41 is witnessed at the public attached-status boundary by a constructor-scoped fault immediately after no-clobber publication and directory synchronization of the exact approved final state, while still inside commit settlement. Exact independent state bytes and parent hashes prove the contiguous head; settlement returns success, and an equivalent repeated request is read-only. P3-42 is independently witnessed at public initialization: confirmation authors canonical crash-left lock bytes, the real exclusive acquisition is observed exactly once, and stable payload-free `workspace_busy` leaves the lock bytes, mode, device/inode identity, workspace, config, library, session tree, and session entries unchanged. P3-43 uses five independent public attached-status mutations over one valid canonical package. After each public preview and real lock acquisition, the instance-scoped hook edits exactly one materially distinct reference: managed vacancy, external selected original, tailored resume, tailored sidecar, or cover letter. Under-lock revalidation returns the fixed payload-free `workspace_drift`; complete tree and session snapshots prove that only the private-mode user edit remains, every historical authority byte is preserved, no state blesses the new digest, and no revision, temporary, backup, lock, session, Core, send, or unrelated mutation remains. This is local deterministic acceptance, not provider-wire evidence. P3-44 remains deferred as a cohesive cluster: current evidence lacks public-boundary below/at/above witnesses and deterministic post-preview under-lock race witnesses for each independent capacity, plus the required complete-tree/session/history/mode and no-side-effect assertions. The production ceilings inventoried for that matrix are 1,024 root entries, 160 application entries, 64 state revisions, 2,097,152 aggregate managed application bytes, 5,242,880 preview bytes, 16,384 metadata-file bytes, 262,144 vacancy/document bytes, 80 characters per config/application identity label, and 65,536 config-file bytes. The label ceiling is config validation (the write helper may truncate labels), so it requires a public invalid-over-limit input witness rather than assuming the write path accepts it. Existing narrower tests and P3-43's reference-drift race do not establish this matrix; retain the deferral until each dimension and the unused send/session/new-session/tool/Core boundaries are independently covered (except Core where the explicit workflow requires it before mutation).
...
- `tests/workflow/application-readiness.test.mjs` exercises the pure production derivation boundary against every reachable availability row, each non-Available class, original-source outcomes, tailored selection, cover dependency precedence, determinism, and output privacy.
- `tests/workflow/application-readiness-integration.test.mjs` exercises the attached read-only state-to-readiness boundary with independently canonicalized synthetic package and library bytes. It establishes a mixed-chain Ready package, then performs separate exact-parent job-description and selected-original transitions, proving a stale bound letter, `Incomplete 2/3`, applied/closed lifecycle non-bypass, preserved history, private modes, and mutation-free derivation. It adds no cover-letter writer or provider/Core/session path; broader filesystem races and overlay presentation remain #65 and later overlay slices. P3-16 is no longer deferred: `tests/workflow/persistent-overlay-creation.test.mjs` drives registered Applications create against a configured private root, resolves persisted consent before exact preview, and proves canonical package bytes, private modes, restart-like Preparing/Incomplete 0/3 catalog visibility, and the absence of attachment, activation, Core, send, result, prompt, library, and residue effects for denial, cancellation, changed preview, post-preview drift, and success.
...
- `tests/workflow/session-catalog-attach.test.mjs` exercises attaching a validated catalog application from a clean Pi session, then drives the registered Applications overlay against active-branch and replayed non-active-branch claims. It proves another UUID is rejected with the fixed conflict notice and explicit replacement-session offer, decline is side-effect free, and acceptance alone attaches the selected application through replacement-session setup while the original session remains unchanged. Broader overlay presentation remains later.
...
- `tests/workflow/application-status-authority.test.mjs` exercises attached `/career-application` status and clear. Its P3-08/P3-09 witness reads the current status from an independently canonicalized two-revision v1 chain through the registered command, then commits an approved attached status mutation through the application workflow. It snapshots the complete observed application-root, library, and config trees (bytes, names, and modes), independently verifies the exact null-cover v2 transition and requested v2 revision with contiguous sequences and parent SHA-256 values, and proves every prior byte and session entry remains unchanged. The bounded Core invocation spy remains unused; provider/model/network absence is not inferred from unavailable boundaries. Cancellation and clear coverage remain mutation-free.
- `tests/workflow/application-concurrency-acceptance.test.mjs` exercises P3-38 and P3-39 through public workflow methods with independently prepared approvals and separate instance-local mutation queues. A scoped post-lock checkpoint holds the deterministic winner while the contender performs a real exclusive-lock acquisition. The tests independently construct canonical bytes and parent hashes, snapshot the complete synthetic config/root/application/library/session tree, require one stable payload-free `workspace_busy` loser, and prove exact names, private modes, contiguous authority, no retry/fork/overwrite/gap, no stale lock, no session append, and no mutation outside the approved winner. The same file separately witnesses P3-41 exact-state settlement, P3-42 crash-left lock handling, and P3-43 referenced-byte drift.
- `tests/workflow/application-artifact-reconciliation-acceptance.test.mjs` exercises P3-40 through the registered vacancy editor and registered `Status and reconcile` action. Its instance-scoped post-artifact/pre-state checkpoint leaves one exact private orphan after publication durability; restart-like reconstruction, catalog projection, and attached-source validation prove mutation-free quarantine with no implicit adoption. Complete synthetic trees, historical bytes, sessions, modes, and available Core/send/new-session/append/tool boundaries remain accounted for.
- `tests/workflow/unknown-child-attachment-boundary.test.mjs` witnesses P3-05 through registered configure-root and `/career` Applications discovery/attach. A marked private root contains one valid application, one legacy application, one colliding UUID pair, and one unknown direct child. Catalog classification keeps the unknown child aggregate drifted and the collision `duplicate_id`; neither is listed or attachable. Repeated configure, detach, status, and attach actions preserve every config, root, application, library, and session name, byte, SHA-256 value, and private mode. The registered workspace action surfaces the existing payload-free error instead of discarding it. No Core resolution, send, activation, new session, repair, or adoption occurs.
- `tests/workflow/overlay-exact-label-restart.test.mjs` witnesses P3-06 and P3-07 through the registered `/career` Applications list and local open/detail surfaces. Independently canonical identity bytes, not directory slugs, supply exact company/role labels after restart-like reconstruction. Distinct UUIDs with Unicode-equivalent byte-different labels stay separate in RPC options and TUI render without normalization or disambiguation suffixes. Repeated reads preserve config, root, application, library, and session names, bytes, and private modes. Colliding and malformed children remain existing duplicate/drifted classifications and are not repaired or adopted. Core, send, append, reload, new-session, and tool-activation spies stay unused. This is local synthetic acceptance, not provider-wire or process-shutdown evidence.
- `tests/workflow/legacy-overlay-migration-acceptance.test.mjs` cohesively witnesses P3-11 through P3-15 and P3-47 through the registered Applications overlay and public workspace migration method. Independently canonical synthetic manifest, v1 state, vacancy, assisted artifact, sidecar, config, root, library, and session bytes are snapshotted with every name and mode. Identity-less list/open, preview cancellation, edited preview, and deterministic post-preview referenced-byte drift create no identity/state file. Confirmed migration without session identity or attachment publishes only the exact canonical private identity; concurrent independently prepared plans have one no-clobber winner with no retry, alternate, fork, or residue. Available Core, send, session append, new-session, and tool boundaries remain unused. This is local synthetic acceptance, not provider-wire evidence. Identity-only migration remains distinct from mutation-triggered v1→v2 state upgrade.
- The earlier `P3-08` reconciliation test in `tests/workflow/application-workspace.test.mjs` remains supporting evidence for repeated v1 reconciliation, while the #63 session-bound cases remain regressions for exact identity-only migration and state/session races.
- `tests/workflow/transient-process-shutdown-acceptance.test.mjs` witnesses P3-30 with two real `pi` child processes. The first loads `dist/index.js` through public RPC, attaches and activates a synthetic application using only in-memory session entries, leaves `sessionFile` absent, does not submit the editor handoff, and is stopped with SIGTERM. The fresh process has a different pid and no attachment, activation, Career Skill, or managed tool surface. Approved config, root, application, and library names, bytes, hashes, and private modes match. No session JSONL, Core invocation, agent turn, or prompt/result byte is persisted. P3-58 stays deferred: built-in TUI `/reload` is not an RPC command, and activation's own `ctx.reload()` is not an independent post-activation reload.
- `tests/workflow/no-automatic-provider-submission.test.mjs` witnesses P3-32 and P3-49 through real installed RPC children and a reachable loopback provider trap. P3-49 browses, opens, filters, clears, observes an empty result, and repeats reads against a valid catalog without attachment; its context/effect assertions cover custom entries, Career Skill metadata/content, Career tool schemas, Core calls, provider calls, and exact private trees. Browsing lists and opens a synthetic catalog application without attachment or session append. Confirmed no-Core mutations cover identity migration, persistent package creation, attached status revision, selected-original binding, and overlay Detach, including preview cancellation, confirm denial, and detach cancellation. Confirmed Detach appends only the exact session detachment, retains the used-application claim, and leaves workspace files unchanged; inactive assistance means this path does not reload. The trap request count stays zero after a positive probe, host agent/turn/message events stay absent, messages stay empty, raw tools stay inactive, and private bytes/modes outside the approved files stay exact. Core-routed vacancy/analyze/match, config-root and library-root mutations (including workspace-root detach), Ask Pi editor handoff, activated detach reload, and process shutdown are not this witness.
- `tests/workflow/payload-free-error-boundaries.test.mjs` witnesses P3-37 through registered commands/tools and one installed RPC child. Distinct synthetic sentinels are injected into resolver probes, Core invocation, source scan/extraction/validation, catalog/config/workspace reads, session loader/replay, UI select/editor/notification failures, persistence/commit failures, and public managed/raw tool errors. Public results are approved stable codes/messages, cancellation remains distinguishable, and stdout/stderr, RPC UI, session entries, result cards, and synthetic config/root/library/package/lock/temp sinks contain none of the injected private material. No logger or telemetry was added.
- `tests/workflow/no-private-sentinel-persistence.test.mjs` witnesses P3-33. Distinct synthetic Core-result, Core-error, provider-request, provider-response, prompt, and editor sentinels stay in process memory while registered create, migrate, and list/browse run and public readiness derivation reads an in-memory sentinel-bearing scan. Complete before/after snapshots cover config, application root, library, session file, result-card append, lock/temp/residue, synthetic package cache, and the writable temp tree. Approved writes are canonical only. Restart-like empty-memory reads reconstruct readiness from disk. The loopback trap observes no workflow request. This does not claim secure erasure or provider-wire behavior beyond that trap.
- `tests/workflow/capacity-boundary-matrix.test.mjs` exercises the reachable P3-44 public plan/commit/read cells: root entries 1,023/1,024/1,025, application entries and 64 revisions through registered status mutation, managed bytes 2,097,151/2,097,152/2,097,153, document reads 262,143/262,144/262,145, metadata size classification, and 80-character config labels plus the separate 120-character application-identity ceiling. Under-lock hooks cover root drift, application-entry `workspace_limit_reached`, revision drift, document-byte `workspace_limit_reached`, and oversized-metadata drift. Preview witnesses use actual canonical encoding and stay below 5,242,880; exactly-at/just-above preview bytes, a schema-valid 16,384-byte metadata file, a 262,144-byte vacancy write, and config/label under-lock injection remain unreachable without new production seams. P3-44 therefore stays deferred. This is local synthetic acceptance, not provider-wire evidence.
- `tests/workflow/inactive-model-context.test.mjs` witnesses P3-50 and P3-57 through isolated installed `pi --mode rpc --offline --no-session` children and a bounded synthetic loopback provider. P3-50 attaches one valid catalog application without assistance activation, then sends one ordinary non-Career prompt. P3-57 calls registered `/career-tools raw` with no valid activation, observes the stable inactive rejection, and proves a post-startup skill probe stays undiscovered so the command does not reload resources. The following ordinary prompt is the first provider request. Tests parse only that local synthetic body and do not print it: the explicit prompt occurs once, host tool schemas stay `read`, `bash`, `edit`, and `write`, and Career Skill discovery metadata, Skill content, `career_run`, and the raw `career_core_discover`, `career_core_resume`, and `career_core_job` schemas are absent. Tests construct the exact canonical identity-file bytes and the exact attachment session-entry JSON, then assert those serializations are absent along with application and library document bytes, the editor handoff, and private paths including the agent directory, home, sessions directory, Core trap, library, workspace, catalog, and identity file. The installed host system prompt includes the process working directory once; that host line is the only working-directory occurrence and is not a package-owned Career leak. The loopback acknowledgement is the assistant message. No Career Core executable runs. This is local synthetic provider-request evidence, not an external-provider witness.
- Other ledger rows remain specifications, not newly implemented tests. The 3 deferred rows require issue-owned behavioral integration; the map intentionally refuses to count fixture-only or narrower read evidence as completed mutation/UI coverage. #60 remains open pending review of these deferrals and further end-to-end coverage.

## Next gate

The exact state-v2/readiness contract, #60/#62 foundations, and #63 migration are implemented. #64's attachment bytes, branch/restart/detach lifecycle, command authority, and context-on-demand mechanism are fixed; parsing/replay, pointer validation, on-demand Skill/tool discovery, explicit attach/detach/assistance-activation commands, catalog attach from any session including the explicit cross-UUID replacement offer, and application/vacancy/Resume command-authority convergence are implemented. Broader overlay UI remains later. #57 retains cover-letter writes. Keep #60 open until remaining filesystem-race/integration rows are exercised or explicitly deferred in review.
