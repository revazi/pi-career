...
Consent is specific and non-transitive. Gate 1 currently exercises session persistence, provider submission, and the file-mutation classes below. The attachment and model-context decisions are fixed here for #64. Strict record parsing/replay, fresh pointer validation, and on-demand Skill/tool discovery are implemented; overlay attach/activation commands remain later slices.
