...
- Unattached `/career-application` owns session-scoped application identity and status and creates no workspace file. Once attached, status is shown and updated only through a previewed workspace revision; clear detaches and never appends a competing session-only status or vacancy.
