...
- When no application root is configured, Applications create remains session-scoped and creates no workspace file. When a valid root is configured and the session has no application claim, create collects company/role labels, resolves persisted-session consent before the existing initialize preview, and appends the creation entry only after that package commits. It does not attach or activate assistance. Once attached, status is shown and updated only through a previewed workspace revision; clear detaches and never appends a competing session-only status or vacancy.
...
Consent is specific and non-transitive. Gate 1 currently exercises session persistence, provider submission, and the file-mutation classes below. The attachment and model-context decisions are fixed here for #64. Strict record parsing/replay, fresh pointer validation, on-demand Skill/tool discovery, explicit attach/detach/assistance-activation commands, catalog attach from any session, and attached application/vacancy/Resume command-authority convergence are implemented. A clean Pi session may attach a validated catalog application. If the registered Applications overlay receives an attach request for another UUID, it rejects with a fixed payload-free notice and explicitly offers `Open application in new Pi session`; only acceptance creates the replacement.
...
The one-company/role rule applies to session use, not catalog browsing. A user may browse any number of applications locally. For the irrevocable used-application check, the adapter examines all entries from `SessionManager.getEntries()`, including strict legacy workflow identity entries and valid attachment entries on abandoned branches. The first application UUID claims the session. Any attempt to create or initialize another UUID fails. A public Applications attach request for another UUID fails without append and offers `Open application in new Pi session`; branch navigation, detachment, clearing, or compaction does not release that claim. Same-UUID reattachment after an explicit detach is allowed only after full current validation and gets a fresh `attachment_id`.

Menu initialization still requires the current branch to reconstruct exactly one active legacy application whose UUID is canonical lowercase. Registered Applications create is separate: with a configured valid root and a clean session, it holds the preparing identity in memory, shows the same initialize preview and confirmation, and appends that identity only after durable commit. Cancellation, a changed preview, consent denial, or post-preview root drift publishes no package and leaves no lock or temporary residue. A persisted consent record may already exist before preview; declining consent appends only that existing denial record and does not name the session. Without a configured root, create remains the session-only confirmation path. A matching workspace attachment may coexist with that same legacy identity only when UUID, creation timestamp, and exact labels agree. Once attached, workspace state is authoritative and later commands cannot append a competing session-only status or vacancy. The package never accepts an application UUID from a command argument or file picker and never uses company/role labels alone as authority.

`Open application in new Pi session` uses `ctx.newSession({ parentSession, setup, withSession })`. `setup` appends only a freshly generated attachment record for the selected validated application through the replacement `SessionManager`; it copies no conversation, activation, document, or result entry. A successful replacement tears down the old runtime, starts and validates the new one, and uses only the replacement callback context for a local notification. It does not submit a message. Cancellation or setup/replacement failure leaves the old session and overlay unchanged and appends nothing there.
...
Fields have exactly the order shown. The file uses canonical two-space JSON plus one LF, is at most 16,384 bytes, and has the same strict private-file metadata requirements as the manifest. Labels retain their exact bounded UTF-8 bytes and are never normalized through the slug algorithm. Registered Applications list and local detail render those exact identity bytes; directory slugs are not display authority, and Unicode-equivalent byte-different labels on distinct UUIDs are not normalized or conflated. The UUID and timestamp must match the immutable manifest. The file contains no path, document body, Core/provider result, prompt, credential, session ID, or environment value. Absence is accepted only as the explicit legacy classification; opening or listing a legacy application never creates this file.

A legacy catalog row may expose only the two already validated safe directory slugs with authority `directory_slug_non_authoritative`; it never promotes them to exact labels. The Applications view offers `Finish migration` for a selected legacy UUID and separately asks for exact company and role labels; migration does not require or create a matching session identity or attachment. The entered labels must derive the already-existing canonical directory basename, while the UUID and creation timestamp come only from the validated manifest. The existing session-bound workspace action remains available for an exact matching branch identity. Both paths preview and separately confirm one no-clobber identity-file creation. Cancellation, edited preview, capacity failure, changed session/config/root/application evidence, collision, or non-exact settlement creates no identity or state revision and never rewrites an existing byte. Only the publishing transaction may settle its exact identity as success; concurrent independently approved plans have one winner. Listing and opening remain read-only.
