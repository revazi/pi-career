...
**Implementation status:** Gate 1, the approved #61 read-only identity/catalog foundation, and the first #62 mixed-state reader slice are implemented in the current unreleased source for independent security/architecture review. They include strict config-v2 migration/CAS, one existing private disjoint application root, marker attach/detach, immutable manifest/display identity and full state/vacancy/selected-original revisions, bounded read-only catalog derivation, fail-closed all-v1/all-v2/v1→v2 chain validation, and read-only reconciliation through user-only `/career-workspace`. Readiness derivation and every v2 writer remain unimplemented. This status does not authorize merge, release, publication, tagging, or any later gate.
...
The reviewed next-schema decision is documented in [the exact state-v2 and derived-readiness contract](persistence-acceptance.md#exact-state-v2-and-derived-readiness-contract). It adds only `cover_letter_artifact` after `resume_artifact`, defines mixed v1/v2 reading and pure readiness, and leaves every v1 byte valid and immutable. The current reader accepts only complete all-v1, all-v2, or one-way v1→v2 chains and validates every historical managed reference. Existing writers remain v1-only and fail closed on a v2 head until separately reviewed #63 transactions exist. This reader does not derive readiness or authorize migration, attachment, cover-letter writes, or overlay UI.
