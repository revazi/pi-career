...
**Implementation status:** Gate 1, the approved #61 read-only identity/catalog foundation, and the two #62 read slices are implemented in the current unreleased source for independent security/architecture review. They include strict config-v2 migration/CAS, one existing private disjoint application root, marker attach/detach, immutable manifest/display identity and full state/vacancy/selected-original revisions, bounded read-only catalog derivation, fail-closed all-v1/all-v2/v1→v2 chain validation, and pure component/readiness derivation over an already validated snapshot plus complete bounded scan evidence. No catalog/detail UI calls that derivation yet, and every v2 writer remains unimplemented. This status does not authorize merge, release, publication, tagging, or any later gate.
